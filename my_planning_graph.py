...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        if node_a1.prenodes == node_a2.effnodes or node_a1.effnodes == node_a2.prenodes :
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for pre_cond in node_a1.precond_s_nodes():
            if pre_cond in node_a2.effnodes:
                return True

        for pre_cond in node_a2.precond_s_nodes():
            if pre_cond in node_a1.effnodes:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for s1_par in node_a1.parents:
            for s2_par in node_a2.parents:
                if not s1_par.is_mutex(s2_par):
                    return False

        #return node_a1.action.precond_pos.is_mutex(node_a2.action.precond_pos)
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        return not (node_s1.is_pos == node_s2.is_pos and node_s1.symbol != node_s2.symbol)
    ...
    def h_levelsum(self) -> int:
        ...
        #goal = self.problem.
        sLevelSize = len(self.s_levels)
        print(sLevelSize)
        for r in range(0, sLevelSize):
            for state in self.s_levels[r]:
                print(goal, state.lite, 'Hello', r)
                if goal == state:
                    return r
                level_sum +=r
